...
public class LIRTranslatorOptimizations implements PropagatingVisitor<DownType, UpType>{
	...
	//Start of visitor
	public LIRTranslatorOptimizations(LayoutsManager layoutManager){
		variableUnique = 0;
		parameterUnique = 0;
		fieldUnique = 0;
		labelUnique = 0;
	...
	public UpType visit(VariableLocation location, DownType d) {
		...
		boolean resultInRegister = d.resultInRegister;
		d.prevNode = location;
		UpType upTypeReturned = new UpType();
		if(location.isExternal()){
			//Must Be A Field
			d.loadOrStore = false; //load
			d.resultInRegister = true;
			UpType upType = location.getLocation().accept(this,d);
			d.resultInRegister = resultInRegister;
			if(upType == null) return null;
			...
			if(loadOrStore){
				...
				d.freeRegister(upType.getTarget());
			}else{
				//Load
			...
			if(rec.getKind() == Kind.Field){
				...
				d.freeRegister(thisField.getTarget());
			}
			...
			if(rec.getKind() == Kind.Local_Variable || rec.getKind() == Kind.Parameter){
				...
				if(loadOrStore){
					...
					//Load
					if(d.resultInRegister){
						String reg = d.nextRegister();
						upTypeReturned.setTarget(reg);
						instructions.add(spec.Move(name, reg));
					}else{
						upTypeReturned.setTarget(name);
					}
				}
			}
